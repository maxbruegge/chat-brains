...
class NetworkClient(
    ...
    suspend fun branches(): Result<Array<String>, NetworkError> {
        val response = try {
            httpClient.get(
                urlString = "http://localhost:8000/api/github/branches"
            ) { }
        } catch(e: UnresolvedAddressException) {
            return Result.Error(NetworkError.NO_INTERNET)
        } catch(e: SerializationException) {
        ...
        return when(response.status.value) {
            in 200..299 -> {
                val branchesResponse = response.body<BranchesResponse>()
                if (branchesResponse.success) {
                    Result.Success(branchesResponse.branches)
                } else {
                    Result.Error(NetworkError.UNAUTHORIZED)
                }
            }
